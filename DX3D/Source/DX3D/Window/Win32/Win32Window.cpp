...
static LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_KEYDOWN:
	{
		InputSystem::onKeyDown(static_cast<unsigned int>(wparam));
		break;
	}
	case WM_KEYUP:
	{
		InputSystem::onKeyUp(static_cast<unsigned int>(wparam));
		break;
	}
	case WM_CLOSE:
	{
...
dx3d::Window::Window(const WindowDesc& desc) : Base(desc.base), m_size(desc.size)
{
	...
	m_handle = CreateWindowEx(NULL, MAKEINTATOM(windowClassId), L"Eksdi Particles",
